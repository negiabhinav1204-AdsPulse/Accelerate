...
import { redis, orgKey, TTL } from '../lib/redis';
...
export async function reportRoute(fastify: FastifyInstance) {
  fastify.get('/reporting', async (request, reply) => {
    // Auth: INTERNAL_API_KEY (dashboard already verified session + membership)
    ...
    const cacheKey = orgKey(orgId, `reporting:${dateRange}:${campaignId ?? 'all'}:${dateFrom}:${dateTo}`);
    try {
    ...
    for (const report of reports) {
      ...
      for (const row of reportRows) {
        if (row.date_start >= dateFrom && row.date_start <= dateTo) rows.push(row);
      }
    }
    ...
    // summaryTotals
    let totalSpend = 0, totalImpressions = 0, totalClicks = 0, totalConversions = 0, totalRoas = 0, roasCount = 0;
    for (const row of rows) {
    ...
    // perCampaign
    const campaignMap = new Map<string, { campaignId: string; campaignName: string; spend: number; impressions: number; clicks: number; conversions: number; roasSum: number; roasCount: number }>();
    for (const row of rows) {
      const existing = campaignMap.get(row.campaign_id) ?? { campaignId: row.campaign_id, campaignName: row.campaign_name, spend: 0, impressions: 0, clicks: 0, conversions: 0, roasSum: 0, roasCount: 0 };
      const roas = getRoas(row);
    ...
    for (const report of ageGenderReports) {
      ...
      for (const row of reportRows) {
        if (row.date_start >= dateFrom && row.date_start <= dateTo) {
          ...
          if (gender === 'female') ageGenderMap.set(row.age, { ...existing, female: existing.female + clicks });
          else if (gender === 'male') ageGenderMap.set(row.age, { ...existing, male: existing.male + clicks });
          else ageGenderMap.set(row.age, { ...existing, others: existing.others + clicks });
        }
      }
    }
    ...
    for (const report of platformReports) {
      ...
      for (const row of reportRows) {
        if (row.date_start >= dateFrom && row.date_start <= dateTo) {
          const pub = (row.publisher_platform ?? '').charAt(0).toUpperCase() + (row.publisher_platform ?? '').slice(1).toLowerCase();
          const pos = (row.platform_position ?? '').replace(/_/g, ' ').replace(/\w\S*/g, (w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase());
          const region = `${pub} ${pos}`.trim();
          const existing = platformMap.get(region) ?? { region, conversions: 0 };
          platformMap.set(region, { region, conversions: existing.conversions + parseInt(row.clicks || '0', 10) });
        }
      }
    }
    const platformBreakdown = Array.from(platformMap.values()).sort((a, b) => b.conversions - a.conversions).slice(0, 5);
    ...
    try {
      await redis.setex(cacheKey, TTL.REPORTING, payload);
    } catch {
      // Non-fatal
    }
