    ...
    for (const adType of platform.adTypes) {
      ...
      if (!apiKey) {
        ...
        for (let i = 0; i < Math.min(adType.ads.length, 3); i++) {
          ...
          // Always use AI generation with lifestyle creative prompts.
          // creativeImagePrompts are generated by the Creative Agent and include
          // audience, theme (seasonal/event), location, and product context.
          // Product images are used only as fallback if AI generation fails.
          ...
          // Fallback to product image only if AI generation failed
          if (!generated && realProductImages.length > 0) {
            imageUrls.push(realProductImages[i % realProductImages.length]!);
          }
        }
      }
