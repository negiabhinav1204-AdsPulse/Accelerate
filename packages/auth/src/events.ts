...
export const events = {
  async signIn({ user, account, profile, isNewUser }) {
    if (user && user.id) {
      // Update lastLogin; mark isFirstLogin=false on subsequent logins
      ...
      await prisma.user.update({
        ...
        data: {
          lastLogin: new Date(),
          // Set to false after first login recorded
          isFirstLogin: existingUser?.isFirstLogin === true ? false : false
        },
        select: {
          id: true // SELECT NONE
        }
